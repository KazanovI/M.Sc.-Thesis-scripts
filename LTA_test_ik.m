function [data_both,inv_cells_sess,ret_trial_track,inv_trials_sess_both...
    ,cnt_licks,ret_licks_time,ret_m_fr_sess,ind_resp] = LTA_test_ik(Tosca_F)
    % extract trials and check conditions
    [TF,trial_track] = extract_conds(Tosca_F,1);
    % Sound responsive cells
    [ind_resp] = sound_resp_cells(TF);
    % extrcat LTA and licks data
    [sess_LTA_start,inv_trials_sess_start,cnt_licks_start] = LTA_start(TF);
    [sess_LTA_task,inv_trials_sess_task,cnt_licks_task,licks_time] = LTA_task(TF,0);
    ...
    ret_trial_track = [{trial_track};{trial_track}];
